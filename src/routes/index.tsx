...
import { schemes, notices, faqs } from "@/lib/mock";
...
const steps = [
  { icon: UserPlus, title: "Register", desc: "Create an account with your mobile number and email, then verify with a one-time password." },
  { icon: IdCard, title: "Complete profile", desc: "Add personal, caste, academic, institution and bank details once — reused across every application." },
  { icon: FileText, title: "Apply to a scheme", desc: "Fill the guided multi-step form, save drafts anytime and review before submission." },
  { icon: FileCheck2, title: "Upload documents", desc: "Upload the checklist for your scheme. Replace any file if a deficiency is raised." },
  { icon: Landmark, title: "Verification", desc: "Your institution confirms enrolment and Ministry officers scrutinise the application." },
  { icon: BadgeCheck, title: "Selection & award", desc: "Selection and approval decisions are published in your portal with the award details." },
];
      ...
      <section className="shell py-16 md:py-20">
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {schemes.slice(0, 6).map((s) => (
            <Card key={s.id} className="flex flex-col shadow-card transition-shadow hover:shadow-lift">
      ...
      <section className="border-y bg-surface py-16 md:py-20">
        <div className="shell">
          ...
          <ul className="divide-y overflow-hidden rounded-xl border bg-card">
            {notices.map((n) => (
              <li key={n.id} className="flex flex-col gap-2 p-5 sm:flex-row sm:items-center sm:gap-6">
